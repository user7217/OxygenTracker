...
@app.route('/cylinders/borrow/<cylinder_id>')
@login_required
def borrow_cylinder_form(cylinder_id):
    """Show form to borrow a cylinder"""
    cylinder = cylinder_model.get_by_id(cylinder_id)
    if not cylinder:
        flash('Cylinder not found', 'error')
        return redirect(url_for('cylinders'))
    ...
    if cylinder.get('status', '').lower() != 'available':
        flash('Cylinder is not available for borrowing', 'error')
        return redirect(url_for('cylinders'))
    
    customers = customer_model.get_all()
    return render_template('borrow_cylinder.html', cylinder=cylinder, customers=customers)

@app.route('/cylinders/borrow/<cylinder_id>', methods=['POST'])
@login_required
def borrow_cylinder(cylinder_id):
    """Process cylinder borrowing"""
    cylinder = cylinder_model.get_by_id(cylinder_id)
    if not cylinder:
        flash('Cylinder not found', 'error')
        return redirect(url_for('cylinders'))
    ...
    try:
        ...
        # Convert borrow_date if provided, otherwise use current time
        if borrow_date:
            try:
                borrow_datetime = datetime.fromisoformat(borrow_date).isoformat()
            except:
                borrow_datetime = datetime.now().isoformat()
        else:
            borrow_datetime = datetime.now().isoformat()
        
        # Update cylinder with borrowing information
        cylinder_data = {
            'status': 'Rented',
            'rented_to': customer_id,
            'customer_name': customer.get('name', ''),
            'customer_email': customer.get('email', ''),
            'date_borrowed': borrow_datetime,
            'date_returned': '',
            'rental_date': borrow_datetime,
            'notes': cylinder.get('notes', '') + f"\nBorrowed: {notes}" if notes else cylinder.get('notes', ''),
            'updated_at': datetime.now().isoformat()
        }
        
        # Preserve existing cylinder data
        for key, value in cylinder.items():
            if key not in cylinder_data:
                cylinder_data[key] = value
        
        updated_cylinder = cylinder_model.update(cylinder_id, cylinder_data)
        if updated_cylinder:
            flash(f'Cylinder {cylinder["serial_number"]} borrowed by {customer["name"]} successfully', 'success')
        else:
            flash('Error processing cylinder borrowing', 'error')
            
    except Exception as e:
        flash(f'Error borrowing cylinder: {str(e)}', 'error')
