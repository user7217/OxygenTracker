...
## Changelog
- July 24, 2025: Implemented comprehensive rental history system with active/past dispatch tabs in customer details page. Added 6-month automatic cutoff for old rental returns during imports to maintain performance. Created rental history export functionality in reports section. Enhanced instant importer to handle customers, cylinders, and transactions with unified ultra-fast processing - all imports now use zero-overhead instant mode for maximum performance. Fixed modal flickering in customers page by replacing active dispatches modal with dedicated page. Updated all CSV and PDF export reports to use custom IDs instead of system-generated IDs, changed "Custom ID" headers to just "ID", removed redundant system ID columns from exports, and enhanced all customer and cylinder reports to show display IDs (custom ID priority) instead of internal system IDs. Fixed KeyError 'name' issue in cylinder dispatching by updating customer field handling to support both old and new customer data structures. Added get_display_id method to prioritize custom IDs over system IDs in all cylinder displays. Updated all cylinder flash messages and table displays to show custom IDs instead of system-generated IDs. Enhanced customer field compatibility throughout the application for seamless data handling.
