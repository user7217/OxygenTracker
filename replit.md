...
## Changelog
- July 28, 2025: Created complete MySQL migration system for professional PythonAnywhere hosting. Added app_mysql_clean.py (clean MySQL Flask app), migrate_sqlite_to_mysql.py (SQLite to MySQL migration), and MYSQL_PYTHONANYWHERE_SETUP.md (complete MySQL setup guide). Migration preserves all data (249 customers, 6,774 cylinders, 10,000 rental records) while providing better performance, concurrent access, and professional database features. Fixed all template routing issues and created comprehensive deployment documentation.
- July 25, 2025: Created comprehensive PythonAnywhere deployment setup using SQLite for free account compatibility. Added app_sqlite.py (Flask app for SQLite), sqlite_models.py (SQLite-optimized database models), wsgi.py (PythonAnywhere WSGI config), import_to_sqlite.py (JSON to SQLite migration), and PYTHONANYWHERE_DEPLOY.md (complete deployment guide). SQLite setup eliminates need for paid database hosting while maintaining all application features. Perfect for free PythonAnywhere hosting with simple single-file database backup and deployment.
...
- July 24, 2025: Fixed cylinder table sorting issues - rented cylinders now appear first (by rental duration), followed by available cylinders. Updated rental duration filters to match frontend dropdown values (under_1, 1_to_3, 3_to_6, 6_to_12, over_12). Fixed date_borrowed cleanup when cylinders are returned to prevent sorting confusion. Enhanced SQLAlchemy queries with proper case-based sorting logic.
