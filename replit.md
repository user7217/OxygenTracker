...
## Changelog
- July 21, 2025: Created comprehensive standalone data importer with command-line interface for MS Access database imports. Features include unique ID generation (CUST-XXXXXXXX for customers, CYL-XXXXXXXX for cylinders), field mapping system, duplicate detection, and transaction linking. Updated cylinder import requirements: custom ID (renamed to just "id") is now REQUIRED while serial number is optional. This aligns with bulk operations where memorable IDs like "A1", "B2", "001" are preferred over manufacturer serial numbers. Includes test script, batch import tool, configuration files, and comprehensive documentation.
